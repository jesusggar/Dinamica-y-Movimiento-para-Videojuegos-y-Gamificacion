...
while sim.actualizar_eventos():
	...
	for _ in range(substep):
		...
		v_mod=v.length
		uv=v.normalized()      #vector unitario direc. v
		un=Vec2d(-uv.y,uv.x)   #vector unitario normal a v hacia arriba
		if v_mod>0.1:
			alpha_vel=np.degrees(np.arctan2(v.y,v.x))
			alpha=theta+alpha0-alpha_vel
		else:
			alpha=alpha0
			
		utheta=Vec2d(np.cos(np.radians(theta)),np.sin(np.radians(theta)))
	
		#print(utheta,uv,un)
			
		
		Fe=THRUST*Fe_max        #Empuje
		Fd=fuerza_drag(v_mod,alpha) #Drag
		Fl=fuerza_lift(v_mod,alpha) #Lift
		Fr=fuerza_rodadura() if pos.x==0 else 0  #Rodadura

		
		#suma vectorial de la fuerza
		fuerza=Fe*utheta-Fd*uv+Fl*un+masa*g
		if (fuerza.y<0 and pos.y==0): fuerza=Vec2d(fuerza.x,0)
		
		#Momento de giro de la cola
		C_cola=STICK*C_cola_max
		f_cola=-0.5*rho*A_cola*C_cola*v_mod**2
		M_cola=f_cola*r_cola
		#Momento de estabilidad natural
		C_m_alfa = 1.2  # Pendiente de restauración aerodinámica
		M_estabilidad = -0.5 * rho * v_mod**2 * A_cola * r_cola * C_m_alfa * np.radians(alpha - alpha0)
		#Momento de damping
		if v_mod > 0.1:
			M_damping = -0.5 * rho * v_mod**2 * A_cola * (r_cola**2) * (omega / v_mod)*5.5 #(factor a ojo para que no oscile)
		else:
			M_damping = 0.0
		
		
		M=M_cola+M_estabilidad +M_damping #momento total que actúa
